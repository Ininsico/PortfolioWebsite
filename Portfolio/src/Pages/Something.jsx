...
const CardScanner = () => {
  ...
  useEffect(() => {
    // All the original JavaScript code exactly as provided
    const codeChars =
    ...
    // Cleanup function
    return () => {
      if (controllers.particleSystem) {
        controllers.particleSystem.destroy();
      }
      if (controllers.particleScanner) {
        controllers.particleScanner.destroy();
      }
    };
  }, []);

  // Event handlers that call the original functions
  const toggleAnimation = () => {
    window.cardStream?.toggleAnimation();
  };

  const resetPosition = () => {
    window.cardStream?.resetPosition();
  };

  const changeDirection = () => {
    window.cardStream?.changeDirection();
  };
  ...
  return (
    <div className="card-scanner">
      ...
      <div className="speed-indicator">
        Speed:{" "}
        <span id="speedValue" ref={speedIndicatorRef}>
          120
        </span>{" "}
        px/s
      </div>
  ...
  const cardStreamController = useRef(null);
  const particleSystem = useRef(null);
  const particleScanner = useRef(null);
